...
def open_file(buffer, filename):
    ...
    uri = mate.vfs.URI(path)

    mime_type = mate.vfs.get_mime_type(path) # needs ASCII filename, not URI
    if mime_type:
...
def finished_cb(job, *args):
    ...
    preview = mateprint.ui.JobPreview(gjob, 'PyGtkSourceView Demo Print Preview')
    preview.show()
